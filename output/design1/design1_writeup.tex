...
\graphicspath{{./}{../design2/}{../crim_entities/}}
...
Table~\ref{tab:summary_stats} summarizes the estimation samples: the
ring-design sale sample, the Red Atlas tract price panel, the HMDA purchase
panel, and the 2010 PRCS baseline tract characteristics used for balance
and robustness.
...
\begin{figure}[htbp]
  ...
  \caption{\textbf{Estimated contrast by local investor concentration.}
  Separate event studies for events with 1--4 vs.\ 5+ total identified
  purchases within 1\,km (the event itself included), matching the tract
  design's dose split. Because purchases cluster at the enclave scale (condo
  buildings, resort communities), other events in dense areas fall
  disproportionately \emph{inside} the near ring, so the near--far contrast
  there reflects cumulative local investor exposure rather than a single
  purchase. Low-concentration events show no price effect (post$-$pre
  $-2.7$, s.e.\ $3.5$); 5+ areas show $+6.4$ (s.e.\ $1.5$): single-purchase
  channels (comps entry, one renovation, signaling) appear weak on their
  own, and the price response is a phenomenon of investor
  \emph{concentration} --- the same 1--4/5+ pattern as the tract design.
  Pre-period coefficients are flat in both groups, inconsistent with
  investor waves selecting into already-appreciating pockets.}
  \label{fig:dose}
\end{figure}
...
\begin{figure}[htbp]
  ...
  Table~\ref{tab:censoring} extends the cut to 2020+ and 2022+ cohorts: the
  effect is $5.3$, $5.6$, $6.6$, $4.3$ log points (all significant) as the
  censoring window shortens --- if anything strengthening, and holding for
  the 653 events of 2022+ that have essentially no stale pre-period.}
  \label{fig:late}
\end{figure}
...
Table~\ref{tab:balance_prcs2010} compares treated and never-treated tracts
on 2010 PRCS characteristics, all pre-determined relative to Act 22.
Treated tracts are richer (median income $+\$5{,}700$), higher-value
($+\$39{,}000$), more educated, less poor, and markedly more seasonal-home-
and mainlander-oriented --- exactly the profile of investor selection into
coastal-amenity areas. Residualizing out municipio fixed effects shrinks
the gaps only modestly (15--30\%; all remain significant): treated tracts
are distinct \emph{within} municipios, not just across them. Selection on
2010 observables is a fact about \emph{where} investors go; the
specification ladders below (Table~\ref{tab:controls_robustness}) absorb
municipio-year shocks directly, and the dose exhibits show the resulting
effect heterogeneity explicitly.

\input{../tables/tab_balance_prcs2010.tex}
...
Table~\ref{tab:controls_robustness} subjects the tract-level estimates ---
prices, borrower income, and purchase counts by ethnicity --- to a common
specification ladder, all estimated with the paper's baseline
pre-mean-differenced LP-DiD: (1) calendar-year effects and (2)
county$\times$year effects, so identification is within municipio-year.
The price effect moves from $6.4$ to $5.7$ (both $p<0.01$), borrower
income from $8.1$ to $7.8$ ($p<0.01$), non-Hispanic purchase entry
($100\times$asinh) from $12.7$ to $12.2$ ($p<0.01$), and Hispanic purchase
counts stay a precise null. Table~\ref{tab:ring_controls_robustness} runs
the ladder on the ring design's cell panel (specification (1) reproduces
the baseline estimates exactly), adding in column (3)
house-characteristic controls from the transaction data --- cell means of
log assessed structure value, log lot size, sub-unit share, and
vacant-land share, pre-mean differenced like the outcome. The price effect
goes $5.3$, $4.2$, $3.0$ (all $p<0.01$; the R-squared doubles to $0.53$),
so a constant-composition repricing of roughly $+3$ log points survives
holding fixed what transacts; net ownership conversion is untouched
($2.4$, $2.5$, $2.3$pp, all $p<0.01$). Municipio-year shocks account for
little of any estimate. We do not add 2010 PRCS covariate interactions to
these ladders: ACS suppression drops $\approx$9\% of cells concentrated in
exactly the low-population resort tracts where effects are largest, so
that specification conflates sample composition with adjustment (in a
fixed complete-controls sample the ten interactions move the ring price
estimate by only $\approx$0.5 log points).
Table~\ref{tab:dose_crim} collects the concentration results of both
designs on the common 1--4/5+ dose convention, and
Table~\ref{tab:placebo} reports the placebo contrast in table form.

\input{../tables/tab_controls_robustness.tex}

\input{../tables/tab_ring_controls_robustness.tex}

\input{../tables/tab_dose_crim.tex}
...
(Table~\ref{tab:muni_footprint}).

\begin{table}[H]
\centering
\caption{\textbf{Program Footprint in the Most-Exposed Municipios} This
table reports each municipio's post-2019 housing boom and the share of it
attributable to identified investor purchases under three accountings. All
columns share one valuation convention: each parcel at assessed value times
its tract's median market/assessed ratio (2019+ sales; municipio and island
fallbacks). The dollar boom is the municipio's market stock times
$1-1/(1+g)$, the portion of current value attributable to repeat-sales
growth $g$ since 2019; Uplift is the synthesis in dollars. Shares of boom
connect exactly: \emph{Ring, Total} integrates the distance gradient over
all parcels and \emph{5+ tracts} restricts to parcels in 5+-purchase
tracts; \emph{Tract DiD} applies the dose-specific tract effects to
treated-tract stock (\emph{Total}: all treated tracts; \emph{5+ tracts}:
the high-dose subset). The synthesis \emph{5+ differential} is Tract DiD
(5+) minus Ring (5+) --- the tract-wide repricing beyond what proximity
predicts --- and \emph{Synthesis, Total} $=$ Ring (Total) $+$ the 5+
differential, so the ring layer is never double-counted.}
\scalebox{0.85}{
\onehalfspacing
...
\subsubsection*{Financed mainlander entry}

The non-Hispanic borrower series make the entry margin visible on its own
terms (Figures~\ref{fig:nhpurch}--\ref{fig:nhrefi}; thin base --- a few
hundred non-Hispanic-borrower loans per year island-wide --- hence the wide
intervals). Purchase originations by non-Hispanic borrowers rise $+37$\%
(s.e.\ $14$) pooled and $+41$\% (s.e.\ $19$) at $h\!\ge\!3$ in treated
tracts, with noisy but non-trending pre-periods: decree-era in-movers are
not exclusively cash buyers --- a financed segment follows the same
geography.\footnote{This magnitude reconciles with the ring design's
buyer-composition effect once the two bases are compared. The CRIM
non-Hispanic-\emph{named} buyer share rises $+2.5$pp (s.e.\ $0.7$, PMD
post$-$pre) on a pre-treatment near-ring mean of $18.9$\% --- nominally
$+13$\%. But the surname proxy inflates the base: self-reported non-Hispanic
borrowers are only $1.6$\% of financed purchases in treated tracts
pre-treatment, and non-Hispanic-\emph{named} buyers are $10$\% of the
general market against $67$\% among confirmed investors, so much of the
$18.9$pp proxy base is Hispanic-heritage locals with
non-Hispanic-classifying surnames --- a component inert to treatment. If the
treatment-responsive (true mainlander) share of the proxy base is roughly a
third of it ($6$--$7$pp), the $+2.5$pp effect is $+35$--$40$\% of the
responsive base, matching the HMDA estimate almost exactly. Both sources
thus indicate mainlander purchasing rose by roughly one-third of its
pre-treatment level --- CRIM measuring all transactions at ring geography,
HMDA the financed tail at tract geography.} Their refinancings rise even more steeply --- $+70$\% (s.e.\
$17$) pooled, $+176$\% (s.e.\ $34$) at $h\!\ge\!3$ --- consistent with
newcomers refinancing recently acquired properties as the areas appreciate.
(The corresponding \emph{Hispanic} refinancing series is not usable as
evidence: its pre-period climbs steadily into treatment with no
post-treatment kink, so the incumbent equity-access margin is unidentified
and we do not report it.)

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{../design2/figH9b_purch_nonhisp.png}
  \caption{\textbf{Purchase originations by non-Hispanic borrowers,
  2012--2024 panel.} Poisson event study, treated vs.\ never-treated tracts.
  Pooled $+37$\% (s.e.\ $14$).}
  \label{fig:nhpurch}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{../design2/figH11_refi_nonhisp.png}
  \caption{\textbf{Refinancings by non-Hispanic borrowers, 2012--2024
  panel.} Same population (first-lien owner-occupied 1--4 family; all refi
  types --- the cash-out split does not exist before 2018). Pooled $+70$\%
  (s.e.\ $17$), rising to $+176$\% (s.e.\ $34$) at $h\!\ge\!3$.}
  \label{fig:nhrefi}
\end{figure}

\section{Bounding institutional presence}
\label{sec:institutional}

Our identified investors are name-matched individuals; purchases through
LLCs and trusts are invisible to the name search, and a natural worry is
that the boom's capital is institutional --- ``hedge funds buying Puerto
Rico'' --- rather than decree individuals. CRIM's buyer names let us bound
this directly. We flag corporate-form buyers (LLC/Corp/LP/\ldots) and,
because funds often hold one LLC per property, roll entities up into
\emph{portfolio clusters}: a buyer name, or a set of entity names sharing
one owner mailing address, that accumulates 10+ parcels island-wide (banks,
cooperatives, and GSEs excluded as foreclosure REO).

Three facts bound institutional presence in levels
(Figures~\ref{fig:entshare}--\ref{fig:portshare}). First, entity buyers'
share of purchases rises from 1--2\% before 2012 to 12--15\% by 2025 ---
a decree-era surge. Second, however, the largest entity portfolios are
recognizably local (bank REO stock, a hospital chain, San Juan landlords,
coastal land developers), with no identifiable mainland fund. Third,
purchases by portfolio-scale clusters --- the deeds-data signature a fund
cannot avoid --- never exceed $\approx$2\% of transactions even at the
boom's peak, and the series' one large spike (2014) is a single 480-unit
apartment-building acquisition (Cobi\'an Plaza) predating the boom, which
usefully demonstrates that the measure detects bulk deployment when it
occurs. The entity surge is therefore overwhelmingly small single-LLC
buyers --- consistent with decree individuals wrapping one or two
properties in an entity, and quantifying the match-selection blind spot
rather than overturning it.

The level series cannot say whether the decree shock \emph{attracted}
institutional capital, so we run the paper's tract design with entity and
portfolio purchases as outcomes
(Figures~\ref{fig:entdid}--\ref{fig:portdid}): staggered treatment at the
tract's first identified purchase, never-treated controls. Neither margin
responds. All-entity purchases show a precise Poisson null (pooled
$+12.1$\%, s.e.\ $13.5$): treated tracts' entity purchases sextuple
post-treatment, but never-treated tracts' rise just as fast. Portfolio
purchases, in levels, are bounded at a fraction of one additional purchase
per tract-year (baseline 0.32); the only nonzero point, a wide-CI blip at
$t{=}0$, is five single-building acquisitions coinciding with treatment
onset --- Cobi\'an Plaza alone is 74\% of it --- and vanishes under every
treatment of the outlier (Figure~\ref{fig:portrobust}): dropping those
five tracts ($-0.44$, s.e.\ $0.34$ at $t{=}0$), restricting to 2015+
cohorts (a non-selective cut avoiding the 2014 acquisition: $-0.04$, s.e.\
$0.43$), the extensive margin ($0.003$, s.e.\ $0.035$), and capped counts.
Table~\ref{tab:entity_robust} reports the corresponding pooled effects
with the post window capped at $t{=}2$.

Two scope notes. The design tests the \emph{follow-the-shock} channel ---
funds chasing decree-driven appreciation into its own geography --- which
is the policy-relevant worry; total island deployment is bounded by the
level series, which is geography-agnostic. And ``hedge fund'' identity is
not observable in deeds: attributing specific clusters to funds would
require corporate-registry matching. What the data establish is that
institutional-\emph{scale} buying is small everywhere, did not break with
Act 60, and did not respond to the decree shock where it landed.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{figE1_entity_share.png}
  \caption{\textbf{Entity buyers' share of purchases.} Corporate-form
  buyer names as a share of CRIM-recorded sales $>$\$10{,}000. Last-sale
  censoring would, if anything, overstate early-year shares (entities hold
  longer), understating the rise.}
  \label{fig:entshare}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{figE2_portfolio_share.png}
  \caption{\textbf{Purchases by portfolio-scale entity buyers.} Clusters
  holding 10+/25+ parcels by name or rolled-up mailing address; banks
  excluded. An upper bound on institutional-scale buying: $\leq$2\% of
  transactions in the boom, with the largest episode (2014) predating it.}
  \label{fig:portshare}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{figE3a_entity_did.png}
  \caption{\textbf{Entity purchases around decree treatment.} Poisson
  tract DiD, never-treated controls. The island-wide LLC boom is not
  differentially concentrated where decree buyers land.}
  \label{fig:entdid}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{figE3b_portfolio_levels.png}
  \caption{\textbf{Portfolio-scale purchases: absolute response.} OLS in
  levels; post-year CIs rule out more than $\approx$0.6 additional
  portfolio purchases per tract-year.}
  \label{fig:portdid}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figE4_t0_robustness.png}
  \caption{\textbf{The year-0 blip is five bulk acquisitions.} Robustness
  event studies for the portfolio-purchase DiD; the spike exists only in
  raw counts.}
  \label{fig:portrobust}
\end{figure}

\input{../tables/tab_entity_robust.tex}
...
\begin{enumerate}[leftmargin=*]
  ...
  missing --- but Section~\ref{sec:institutional} bounds the missing mass:
  the entity surge is small single-LLC buyers, and portfolio-scale
  (institutional) purchases are $\leq$2\% of transactions with no response
  to decree treatment.
