...
income result, however, does not have clean pre-trends: the pre-period
coefficients climb monotonically into treatment ($-4.5$, $-3.5$, $0$),
$\approx\!+2.2$ log points per year, and extrapolating that drift accounts
for the majority of the post-period ramp (drift-adjusted effects
$\approx\!+2$ to $+6$); with tract-specific linear trends the pooled effect
falls to $+6.0$ (s.e.\ $3.9$; Hispanic-only $+3.3$, s.e.\ $3.6$), though
trends over-absorb in a seven-year panel with mid-panel adoption. The
honest reading: treated tracts were already on rising borrower-income
trajectories --- consistent with investors selecting gentrifying areas ---
and the post-treatment acceleration beyond that trajectory is positive but
modest and imprecise. Income sorting of the marginal financed buyer is
...
\begin{figure}[htbp]
  ...
  monotone pre-period climb ($\approx\!+2.2$ log points/yr): much of the
  post-period rise extrapolates that drift, so the acceleration attributable
  to treatment is modest (see text).}
  \label{fig:hmdainc}
\end{figure}
