...
\begin{itemize}[leftmargin=*]
  \item[\textbf{Fig.~1}] \textbf{Baseline event study} (\texttt{fig1\_baseline}).
  Near-versus-far log-price coefficients $\{\beta_{-Q},\dots,\beta_H\}$ by year
  relative to purchase. Shows the headline price bump; flat pre-period
  coefficients are the design's core credibility check, and the post-period
  path shows how quickly the premium appears and whether it persists.

  \item[\textbf{Fig.~2}] \textbf{Investor vs.\ placebo purchases}
  (\texttt{fig2\_real\_vs\_placebo}). The real event study overlaid with the
  placebo (non-investor luxury purchases, matched rings). Distinguishes a
  decree-investor-specific effect from a generic response to any high-value
  transaction; because unmatched investors contaminate the placebo
  \emph{toward} finding an effect, a null placebo is conservative evidence of
  investor specificity.

  \item[\textbf{Fig.~3}] \textbf{Spatial gradient} (\texttt{fig3\_gradient}).
  Near-ring (0--250\,m) and gap-ring (250--400\,m) series against the same far
  control. A genuine hyper-local spillover should decay with distance
  ($\text{near} > \text{gap} > 0$); near $\approx$ gap would instead indicate a
  neighborhood-wide shock partially shared by the control ring.

  \item[\textbf{Fig.~4}] \textbf{Dose response} (\texttt{fig4\_dose}).
  Separate event studies for events with 0--2, 3--25, and $>$25 other investor
  events within 1\,km. Larger per-event effects where a single purchase is the
  entire ``dose'' is the pattern a causal effect predicts; the figure also
  bounds the dilution of the pooled estimate from overlapping treatments.

  \item[\textbf{Fig.~5}] \textbf{Censoring robustness} (\texttt{fig5\_late}).
  Baseline overlaid with events from 2018 onward, for which little time exists
  for post-period sales to be overwritten by resales. Agreement with the full
  sample indicates last-sale censoring is not manufacturing the bump.

  \item[\textbf{Fig.~6}] \textbf{Composition checks} (\texttt{fig6\_comp\_*},
  four panels). The identical design with property characteristics as
  outcomes: log assessed structure value, log lot size, condo-unit share,
  vacant-land share. Flat lines imply the price bump is appreciation on a
  stable transaction mix; movements would reveal the stock-upgrading margin of
  gentrification, changing the interpretation of Figure~1.

  \item[\textbf{Fig.~7}] \textbf{Who transacts: buyers vs.\ sellers}
  (\texttt{fig7\_parties}). Effects on the share of nearby sales with
  non-Hispanic-named buyers and, separately, sellers. Buyer share rising
  faster than seller share means the area is net-importing mainland owners
  after an investor arrives --- the composition shift price indices cannot see.

  \item[\textbf{Fig.~8}] \textbf{Transition types} (\texttt{fig8\_transitions}).
  Effects on the shares of transactions that are Hispanic-seller
  $\to$ non-Hispanic-buyer versus non-Hispanic-seller $\to$
  non-Hispanic-buyer. Rising islander$\to$mainlander transitions indicate
  conversion of locally held housing to incomers (the displacement margin);
  rising mainlander$\to$mainlander indicates churn within an
  already-converted segment. Together they distinguish ``investors lead
  conversion'' from ``investors cluster where conversion already occurred.''
\end{itemize}
