...
the same ladder on the ring design's cell panel (specification (1)
reproduces the baseline estimates exactly): price $5.3$ to $4.2$, net
ownership conversion $2.4$ to $2.5$pp (all $p<0.01$). Municipio-year
shocks account for little of any estimate. We do not add 2010-covariate
interactions to these ladders: ACS suppression drops $\approx$9\% of cells
concentrated in exactly the low-population resort tracts where effects are
largest, so the covariate specification conflates sample composition with
adjustment (in a fixed complete-controls sample, the ten interactions
themselves move the ring price estimate by only $\approx$0.5 log points).
