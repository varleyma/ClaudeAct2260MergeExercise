...
Treated tracts are richer, more educated, more mainlander- and
seasonal-home-oriented, and less poor --- large raw gaps, as expected when
investors select coastal-amenity areas. Residualizing out municipio fixed
effects removes most of the imbalance (the treated--never differences
shrink by roughly two-thirds and several lose significance), which is why
the county$\times$year specifications below are the natural robustness
check; Table~\ref{tab:controls_robustness} pushes further, interacting the
full 2010 covariate vector with the boom era.
