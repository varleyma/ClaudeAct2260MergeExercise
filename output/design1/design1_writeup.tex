...
Hispanic --- side of the market. Two results
(Figures~\ref{fig:hmdaeth}--\ref{fig:hmdainc}): the \emph{number} of
Hispanic-borrower purchase originations shows no significant pooled change
($-0.8$\%, s.e.\ $5.3$), though it drifts negative at longer horizons
($-16.0$\%, s.e.\ $8.2$ at $h\!\ge\!3$); and the \emph{income} of purchase
borrowers rises after treatment --- pooled $+10.7$\% (s.e.\ $2.6$), reaching
$+12.6$\% by $h\!\ge\!3$, nearly identical for Hispanic borrowers alone. The
income result, however, does not have clean pre-trends. With the pre window
extended to $-4$ (identified off 2022+ adopters), the pre path is $-4.5$,
$-4.5$, $-3.5$, $0$: flat between $-4$ and $-3$, with the climb concentrated
in the final two pre-years --- less a constant trend than a run-up
immediately preceding the first purchase, consistent with investors entering
tracts where borrower income had just started rising (or with unidentified
investor activity predating our first matched purchase). Under a
linear-trend reading, extrapolated drift accounts for the majority of the
post-period ramp; with tract-specific linear trends the pooled effect falls
to $+6.0$ (s.e.\ $3.9$; Hispanic-only $+3.3$, s.e.\ $3.6$), though trends
over-absorb in a seven-year panel with mid-panel adoption. Either way,
selection on income trajectory is evident, and the post-treatment
acceleration beyond it is positive but modest and imprecise. Income sorting
of the marginal financed buyer is suggestive, not established.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{../design2/figH4_eth_counts.png}
  \caption{\textbf{Purchase originations by borrower ethnicity.} Poisson
  event studies for Hispanic and non-Hispanic borrower purchase counts in
  treated tracts. Non-Hispanic counts are thin island-wide (a few hundred per
  year), hence the wide intervals.}
  \label{fig:hmdaeth}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.85\textwidth]{../design2/figH5_eth_income.png}
  \caption{\textbf{Income of purchase borrowers.} Effect on
  $100\times\ln$(mean borrower income) among purchase originations in treated
  tracts, all classified borrowers and Hispanic borrowers only. Note the
  pre-period path: flat from $-4$ to $-3$, then climbing over the final two
  pre-years --- a run-up into treatment rather than a constant trend; the
  acceleration attributable to treatment is modest (see text). The $-4$
  coefficient is identified off 2022+ adopters only.}
  \label{fig:hmdainc}
\end{figure}
